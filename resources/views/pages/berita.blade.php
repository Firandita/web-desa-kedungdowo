...
<div id="beritaModal" class="fixed inset-0 z-50 hidden bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
  <div class="bg-white rounded-2xl max-w-2xl w-full overflow-hidden shadow-2xl border border-[var(--sawah)]/20 relative flex flex-col max-h-[90vh] transform scale-95 transition-transform duration-300" id="beritaModalCard">
    ...
    {{-- Tombol Close (tetap nempel di pojok, di LUAR area scroll) --}}
    ...
    {{--
      SATU area scroll untuk foto + konten (digabung) supaya foto ikut
      kegeser saat discroll, dan [overscroll-behavior:contain] mencegah
      scroll "bocor" ke halaman belakang begitu mentok di atas/bawah.
    --}}
    <div class="overflow-y-auto flex-1 [overscroll-behavior:contain]">

      {{-- Gambar Header Modal --}}
      <div class="relative w-full aspect-video bg-slate-100 shrink-0">
        <img id="beritaModalImage" src="" alt=""
             class="w-full h-full object-cover ring-1 ring-inset ring-black/5"
             onerror="this.classList.add('hidden'); this.nextElementSibling.classList.remove('hidden'); this.nextElementSibling.classList.add('flex');">
        {{-- Fallback kalau foto belum ada -- full-bleed, tanpa teks judul yang bisa numpuk badge --}}
        <div class="hidden absolute inset-0 foto-fallback flex-col items-center justify-center gap-1.5 text-[var(--sawah-dark)]/40">
          <span class="material-symbols-outlined text-3xl">image</span>
      ...
      <div class="p-6">
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-[var(--sawah)]/10">
...
<script>
  function openBeritaModal(data) {
    ...
    // Reset state gambar (kalau berita sebelumnya sempat gagal load foto).
    const modalImg = document.getElementById('beritaModalImage');
    modalImg.classList.remove('hidden');
    const modalImgFallback = modalImg.nextElementSibling;
    ...
    // wheel tidak pernah "bocor" ke list berita di belakang overlay.
    // Kompensasi lebar scrollbar yang hilang biar layout tidak "kedut".
